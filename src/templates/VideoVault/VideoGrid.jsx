...
export default function VideoGrid({ homeVideos = [] }) {
  ...
  const collections = homeVideos.filter(v => v.collection && Array.isArray(v.videos) && v.videos.length > 0)
  const individualVideos = homeVideos.filter(v => v.id && v.title && !v.collection)
