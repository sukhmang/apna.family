...
export default function VideoSection() {
  ...
  const getYouTubeEmbedUrl = () => {
    if (!eventData.youtubeVideoId) return null
    // For recorded videos
    return `https://www.youtube.com/embed/${eventData.youtubeVideoId}?rel=0`
  }

  const getYouTubeLoopEmbedUrl = () => {
    if (!eventData.youtubeLoopVideoId) return null
    // For looping videos
    ...
    <Card id="memories">
      <VideosContainer>
        ...
        {eventData.youtubeLoopVideoId ? (
          <VideoWrapper>
            <VideoLabel>Memories: In Loving Memory, {memorialData?.name || 'Loved One'}</VideoLabel>
            <VideoContainer>
              <VideoIframe
